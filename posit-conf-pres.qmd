...
od <- tibble::tribble(
  ~device,         ~lang, ~population_weight, ~experiment_weight, ~p_control, ~p_treat,
  "Desktop",     "English",         0.4,     0.63,        0.1,     0.12,
  "Desktop", "Non-English",         0.2,     0.31,       0.04,     0.02,
  "Mobile",     "English",         0.3,    0.05,       0.08,     0.03,
  "Mobile", "Non-English",         0.1,    0.01,       0.01,     0.01
)

population_weights <- od %>% 
                      select(device, lang, population_weight, experiment_weight) 
...
::: {.fragment .fade-in}
* 🤷🏽‍♂️ My PM wants "lift".  Is this the lift?
```{r}
#| echo: true
...
* And can plot lift using `plot_comparisons`
...
* Using `plot_comparisons` and `plot_predictions` to help tell your story
* Return a ggplot object, add your finishing touches
* Signature is very similar to `avg_comparisons`.
